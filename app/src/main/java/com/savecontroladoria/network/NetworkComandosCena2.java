 ...
 */
public class NetworkComandosCena2 {
    private static NetworkComandosCena2 instance;
    ...
    public NetworkComandosCena2(Context c) {
        mContext = c;
        mRequestQueue = getRequestQueue();
    }

    public static synchronized NetworkComandosCena2 getInstance(Context c) {
        if (instance == null) {
            instance = new NetworkComandosCena2(c.getApplicationContext());
        }
        return (instance);
    }
    ...
    public void execute(final String filtro) {
        ...
        if (VerificaConnection.isConnectedWifi(mContext)) {
            ...
            if (ssid.equals("Seu Madruga")) {
                url = Links.getUrlinternacenario2();

            } else {
                url = Links.getUrlexternacenario2();
            }
        } else {
